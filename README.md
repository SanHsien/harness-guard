...
# 四個讓 AI 守規矩的小工具、九套現成的工作流程，加上一份起手規則檔
...
> **這是一份 fork。** 多了三個攔截工具的 Windows 版、第四個攔截工具，以及一支「用實際執行證明裝好了」而不是假設裝好了的驗證腳本。用 Windows 的話從 [`docs/windows-install.md`](docs/windows-install.md) 開始——底下的主要安裝步驟預設是 macOS 或 Linux，在 Windows 上會無聲失敗。完整改動清單：[`FORK.md`](FORK.md)。
...
先看它打算動哪些檔案、往 `settings.json` 加哪幾行。確認沒問題再拿掉 `--dry-run` 跑一次。

它會自動判斷你的作業系統挑對應版本（Windows 挑 Python 版），**合併**進你既有的
`settings.json` 而不是覆蓋，動手前先備份，寫完再讀回來確認 JSON 還是合法的。重跑不會
重複註冊，已經存在的 skill 資料夾也不會被蓋掉——你調整過的版本比原版值錢。

然後把 Claude Code 整個關掉重開，再跑：

```bash
python scripts/verify-install.py
```

exit code 0 才算裝好。這支腳本不看設定檔臉色，它直接把每個 hook 叫起來執行一次，
看它是不是真的會擋。
...
**三個版本都有。**每一支底下分成 `claude-code/`、`codex/`、`windows/` 三個資料夾，判斷邏輯一模一樣，差別只在跟平台介面有關的那幾行。用哪個工具就裝哪個版本，兩個都用就兩個都裝，帳本是分開的不會打架。

**Windows 一定要用 `windows/` 版**——shell 版靠 `jq`，Windows 原生沒有 `jq`，而它們沒有 `jq` 時會直接放行。詳見 [`docs/windows-install.md`](docs/windows-install.md)。
...
Windows 版多了一件事（本 fork 新增）：它會讀專案根目錄的 `.lint-gate.json`。意思是你可以在全域註冊一次、放著不管——沒有這個檔案的專案完全不受影響，哪個專案要開就在那個專案丟一個檔案，立即生效、不用重啟：

```json
{ "cmd": "python -m pytest -q", "fail": "[1-9][0-9]* (failed|error)" }
```

優先順序是 `.lint-gate.json` > 指令參數 > 環境變數。檔案寫壞了也不會卡住收工，讀不到就當作沒有。

程式碼裡有一段判斷千萬不要刪。沒有那段的話，遇到修不好的錯誤，AI 會陷入「想結束、被擋、再想結束、又被擋」的無限迴圈，整個卡死。這是自己寫這類工具最容易踩的坑，我留了註解說明。

### test-gate-guard：測試紅燈不准出貨（本 fork 新增）

這個要補的洞只有一個字元寬。

AI 在同一條指令裡跑測試然後推上去，中間用 `;` 而不是 `&&` 串接。測試紅燈了，`;` 不管這件事——它照樣往下跑，push 就這樣送出去，收尾訊息還說做完了。這不是假設，這個 hook 就是從那次事故長出來的。

所以它在指令執行前先看指令的形狀：前面有測試、後面有 `git commit` 或 `git push`、中間隔的是 `;` 或換行而不是 `&&`？符合就擋，並建議改用 `&&` 或拆成兩步。

它刻意完全不跑測試。它不可能知道你這個專案用什麼框架，硬猜的結果是誤報加上每次都變慢。

它也分得出「指令」跟「在講一句話提到指令」的差別。在工作日誌裡寫「我跑了 `pytest ; git push`」那是文字不是指令——這正是它上線第一天的誤報，修正連同證明它不會再犯的回歸測試一起附上：`python hooks/test-gate-guard/tests/run-tests.py`。

純 Python，不需要 `jq`、不需要 shell，Windows、macOS、Linux 跑起來一模一樣。

### 證明真的裝好了（本 fork 新增）

這裡每個攔截工具都有同一種無聲的失敗模式：註冊錯了、或少裝一個相依套件，它不會報錯，只是從此不執行——你以為自己被保護著，其實沒有。

`python scripts/verify-install.py` 用來把這件事講死。它不是讀設定檔然後宣告一切正常；它會餵一份合成 payload 給每個已安裝的 hook，實際執行再檢查回應——claim-guard 在帳本空的時候有沒有擋下「測試通過」？帳本有真實測試紀錄後有沒有放行？lint-gate 檢查失敗時有沒有擋、第二次有沒有放行？裝完跑一次，之後每次改設定再跑一次。
