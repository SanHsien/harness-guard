...
# ----------------------------------------------------------------------------
#
# Editing the list above works, but an installer overwrites this file, and your
# setting goes with it -- silently, and in the direction of "now it blocks
# everything again". So the settings can also live in a JSON file next to the
# installed script, which nothing in this repo ever writes:
#
...
#   }
#
# `enabled: false` turns the hook off while leaving it installed and registered,
# so switching it back on is one word and takes effect immediately.
# The file wins over the list above. A malformed file is ignored and the
# built-in defaults apply -- a typo must not be a way to silently disable a
# guard.
