...
# Editing the list above works, but an installer overwrites this file and takes
# your setting with it -- silently, and in the direction of "blocking again".
# So the settings can also live in a JSON file next to the installed script,
# which nothing in this repo ever writes:
#
#   ~/.codex/hooks/no-emoji-guard.json
#   {
#     "enabled": true,
#     "exempt_path_substrings": ["/my-notes/", "\\vault\\"]
#   }
#
# `enabled: false` turns the hook off while leaving it installed. The file wins
# over the list above; a malformed file is ignored and the defaults apply, so a
# typo cannot silently disable a guard.

CONFIG_ENV = "NO_EMOJI_GUARD_CONFIG"
CONFIG_NAME = "no-emoji-guard.json"


def load_config():
    """Read the sibling config file, if there is a readable one."""
    path = os.environ.get(CONFIG_ENV) or (
...
def _exempt_list(config):
    exempt = config.get("exempt_path_substrings")
    if isinstance(exempt, list) and all(isinstance(s, str) for s in exempt):
        return exempt
    return EXEMPT_PATH_SUBSTRINGS


def _exempt_subtree(path, config):
    return any(s and s in path for s in _exempt_list(config))
...
def main():
    ...
    if tool_name in {"apply_patch", "Bash", "exec", "shell"}:
        ...
        if _exempt_subtree(path, config):
            sys.exit(0)
